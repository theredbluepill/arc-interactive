...
| ll01 | Simulation | 32×32 | 5 | **Generations lock**: Conway Life; **ACTION6** toggles cells (budget); **ACTION5** advances one generation; after exactly **N** steps the live set must equal the target. Levels use **2×2 still-life** targets (stable blocks). See `scripts/render_ll01_gif.py`. | ![ll01](assets/ll01.gif) | • 1–4: No-op • 5: Step CA • 6: Toggle cell |
...
| ml03 | Geometry | 24×24 | 5 | **Fragile mirrors** (ml02 rules): same **move + adjacent mirror** play as **ml02**, but every mirror the beam **reflects from** is **removed** after that shot (failed shots eat optics). Same authored levels as **ml02**; rule difference is consumption. See `scripts/render_ml03_gif.py`. | ![ml03](assets/ml03.gif) | • 1-4: Move • 5: Fire • 6: Mirror |
...
| ml04 | Timing / Laser | 10×10 | 7 | **Stepped bolt**: **ACTION5** advances laser **one cell** per press; **ACTION6** cycles fixed mirror tiles **/ → \\ → empty** (no placement inventory). Optional **`emit_dir`** in level data (0–3 = E/S/W/N). Distinct from **ml01–ml03** continuous-ray + technician/global placement. See `scripts/render_ml04_gif.py`. | ![ml04](assets/ml04.gif) | • 5: Fire • 6: Mirror |
