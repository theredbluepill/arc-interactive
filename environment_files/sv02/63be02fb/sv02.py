...
class Sv02UI(RenderableUserDisplay):
...
def create_level(level_num: int):
    ...
    for cx, cy in shelter_centers:
        for x, y in expand_warm_zone(cx, cy, grid_size, radius=2):
            level_sprites.append(sprites["shelter"].clone().set_position(x, y))
...
class Sv02(ARCBaseGame):
    ...
    def _decay(self):
        ...
        for sprite in self.current_level._sprites:
            if sprite and "shelter" in sprite.tags:
