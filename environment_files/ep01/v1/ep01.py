...
class U(RenderableUserDisplay):
    def __init__(self, pain, cap: int = 40):
        self.p = pain
        self._cap = cap
        self._li = 0
        self._nlv = 1
    ...
    def update(self, pain, level_index=None, num_levels=None, cap=None, visits_here=None):
        self.p = pain
        if cap is not None:
            self._cap = cap
        ...
        if visits_here is not None:
            self._visits_here = max(0, int(visits_here))
    ...
    def render_interface(self, f):
        ...
        if isinstance(f, np.ndarray):
            ...
            f[h - 2, 2] = c
            cap = max(1, self._cap)
            used = min(cap, max(0, cap - self.p))
            n = min(8, (used * 8) // cap)
            for i in range(n):
                f[h - 2, 4 + i] = 8
            # Landings on current cell (next re-entry cost = this count before step).
            vh = min(8, self._visits_here)
            for i in range(vh):
                f[h - 3, 10 + i] = 15
        return f
def spr():
...
class Ep01(ARCBaseGame):
    ...
    def on_set_level(self, level: Level):
        ...
        self._vc = {}
        self._ui.update(
            self._bud - self._pain,
            level_index=self.level_index,
            num_levels=len(levels),
            cap=self._bud,
            visits_here=self._vc.get((self._p.x, self._p.y), 0),
        )
    def step(self):
        dx=dy=0
