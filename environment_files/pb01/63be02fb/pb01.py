...
class Pb01UI(RenderableUserDisplay):
    def __init__(self, on_target: bool) -> None:
        self._on_target = on_target

    def update(self, on_target: bool) -> None:
        self._on_target = on_target
    ...
    def render_interface(self, frame):
        ...
        if not isinstance(frame, np.ndarray):
            return frame
        h, w = frame.shape
        color = 14 if self._on_target else 15
        for dy in range(4):
            for dx in range(4):
...
def make_level(
    ...
    return Level(
        sprites=sprite_list,
        grid_size=grid_size,
        data={"difficulty": difficulty, "step_limit": 60 + difficulty * 15},
    )


levels = [
    make_level((8, 8), (1, 1), (3, 3), (5, 5), [], 1),
    make_level(
        (8, 8),
        (1, 1),
        (3, 3),
    ...
    make_level(
        ...
        (7, 7),
        [
            (5, y) for y in range(10) if y != 4 and y != 5
        ]
        + [(2, 6), (3, 6), (4, 6)],
        4,
    ),
    make_level(
        ...
        (1, 2),
        (6, 6),
        [(3, y) for y in range(8) if y not in (2, 3, 4)]
        + [(4, 2), (4, 4), (5, 3)],
        5,
    ),
]
...
class Pb01(ARCBaseGame):
    """Push the single crate onto the yellow goal pad."""

    def __init__(self) -> None:
        self._ui = Pb01UI(False)
        super().__init__(
            "pb01",
            levels,
    ...
    def step(self) -> None:
        ...
        if sprite and "block" in sprite.tags:
            ...
            if block_behind and ("block" in block_behind.tags or "wall" in block_behind.tags):
                self.complete_action()
                return
