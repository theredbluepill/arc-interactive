"""Erase paint: every floor cell starts yellow; ACTION6 removes paint. Match gray hints exactly (only those cells stay painted). ACTION1–4 no-op."""
...
class Gp02UI(RenderableUserDisplay):
    ...
    def set_click(self, frame_x: int, frame_y: int) -> None:
        ...
        self._click_frames = Gp02UI.CLICK_ANIM_FRAMES
...
levels = [
    ...
    mk([(x, 4) for x in range(8)], [(3, 3), (4, 4)], (8, 8), 4),
    mk([(3, y) for y in range(8)], [(4, 2), (4, 5)], (8, 8), 5),
]


class Gp02(ARCBaseGame):
        ...
        for y in range(gh):
            for x in range(gw):
                ...
                self.current_level.add_sprite(
                    sprites["paint"].clone().set_position(x, y),
                )
        self._sync_ui()
    ...
    def _sync_ui(self) -> None:
        ...
        self._ui.update(matched, max(1, len(self._goal)))
    ...
    def step(self) -> None:
        ...
        hit = next(
            (p for p in self.current_level.get_sprites_by_tag("paint") if p.x == gx and p.y == gy),
            None,
        )
        if hit:
