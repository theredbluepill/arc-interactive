"""Drill rush: ACTION5 breaks one adjacent wall (limited drills); tight global step budget."""
...
class Dr01UI(RenderableUserDisplay):
    ...
    def render_interface(self, frame):
        ...
        if not isinstance(frame, np.ndarray):
            return frame
        h, w = frame.shape
        for i in range(min(self._drills, 8)):
            frame[h - 3, 1 + i] = 11
...
def mk(sl, drill_budget: int, step_budget: int, d: int):
    return Level(
        sprites=sl,
        grid_size=(10, 10),
        data={
...
class Dr01(ARCBaseGame):
    ...
    def on_set_level(self, level: Level) -> None:
        ...
        self._drills = int(self.current_level.get_data("drill_budget") or 2)
        self._steps = int(self.current_level.get_data("step_budget") or 30)
