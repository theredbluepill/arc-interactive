...
class Sy02UI(RenderableUserDisplay):
    ...
    def set_click(self, frame_x: int, frame_y: int) -> None:
        ...
        self._click_frames = Sy02UI.CLICK_ANIM_FRAMES
    ...
    def render_interface(self, frame):
        ...
        if self._click_pos and self._click_frames > 0:
            ...
            phase = Sy02UI.CLICK_ANIM_FRAMES - self._click_frames
            if phase < 8:
...
class Sy02(ARCBaseGame):
    def __init__(self) -> None:
        self._ui = Sy02UI(MAX_MOVES, 0, 0)
        self._placed_blocks = []
    ...
    def _get_mirror_position(self, x: int, y: int) -> tuple:
        stagger = int(self.current_level.get_data("mirror_stagger") or 0)
        mirrored_x = GRID_WIDTH - 1 - x
        if stagger and (y % 2 == 1):
            mirrored_x -= 1
        mirrored_x = max(6, min(GRID_WIDTH - 1, mirrored_x))
        return (mirrored_x, y)
