"""Line weave (shared corridor): paths may share grid cells, but you may not enter a cell another color already used **from a perpendicular direction** (no crossing at shared vertices)."""
...
class Lw02UI(RenderableUserDisplay):
...
class Lw02(ARCBaseGame):
    def __init__(self) -> None:
        self._ui = Lw02UI(0, 1, 0)
        super().__init__(
            "lw02",
            levels,
    ...
    def _perpendicular_on_shared(
        self, gx: int, gy: int, pdx: int, pdy: int, skip: int
    ) -> bool:
        for i, trail in enumerate(self._paths):
            if i == skip or len(trail) < 2:
                continue
            if (gx, gy) not in trail:
                continue
            k = trail.index((gx, gy))
            if k == 0:
                continue
