"""Diagonal torus Lights Out: ACTION6 toggles a cell and all eight neighbors (Chebyshev) with torus wrap; walls block. ACTION1–4 no-op."""
...
class Lo03UI(RenderableUserDisplay):
    ...
    def set_click(self, frame_x: int, frame_y: int) -> None:
        ...
        self._click_frames = Lo03UI.CLICK_ANIM_FRAMES
    ...
    def render_interface(self, frame):
        ...
        if self._click_pos and self._click_frames > 0:
            ...
            phase = Lo03UI.CLICK_ANIM_FRAMES - self._click_frames
            if phase < 6:
...
levels = [
    mk([(0, 0), (1, 0), (0, 1)], [], (4, 4), 1),
    mk([(0, 0), (3, 0), (0, 3)], [], (4, 4), 2),
    mk([(1, 1), (2, 1), (1, 2), (2, 2)], [], (5, 5), 3),
    mk([(0, 0), (1, 1), (2, 2)], [], (5, 5), 4),
    mk([(0, 0), (0, 2), (2, 0), (2, 2), (1, 1)], [], (6, 6), 5),
]


class Lo03(ARCBaseGame):
    ...
    def _torus_orth(self, gx: int, gy: int) -> list[tuple[int, int]]:
        gwx, ghy = self._gw, self._gh
        out: list[tuple[int, int]] = []
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                if dx == 0 and dy == 0:
                    continue
                out.append(((gx + dx) % gwx, (gy + dy) % ghy))
        return out
    ...
    def step(self) -> None:
        ...
        cells = [(gx, gy)] + self._torus_orth(gx, gy)
        for tx, ty in cells:
