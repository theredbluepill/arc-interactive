...
class Sy03UI(RenderableUserDisplay):
    ...
    def set_click(self, frame_x: int, frame_y: int) -> None:
        ...
        self._click_frames = Sy03UI.CLICK_ANIM_FRAMES
    ...
    def render_interface(self, frame):
        ...
        if self._click_pos and self._click_frames > 0:
            ...
            phase = Sy03UI.CLICK_ANIM_FRAMES - self._click_frames
            if phase < 8:
...
def make_level(pattern_positions: list, difficulty: int):
    ...
    for x in range(GRID_WIDTH):
        sprites.append(DIVIDER_SPRITE.clone().set_position(x, CENTER_Y))
...
# Patterns in rows 0–4 only (divider y=5); mirror targets in rows 6–10.
levels = [
    make_level([(2, 3), (5, 2), (8, 4)], 1),
    make_level([(1, 2), (4, 3), (7, 1), (9, 4), (3, 0)], 2),
    make_level([(1, 1), (3, 3), (5, 0), (8, 2), (9, 4), (2, 4)], 3),
    make_level([(1, 2), (4, 1), (6, 4), (9, 3), (2, 0), (7, 2), (8, 0)], 4),
    make_level(
        [(1, 1), (3, 2), (5, 4), (7, 1), (9, 3), (2, 3), (4, 0), (8, 4), (6, 2)],
        5,
    ),
]


class Sy03(ARCBaseGame):
    def __init__(self) -> None:
        self._ui = Sy03UI(MAX_MOVES, 0, 0)
        self._placed_blocks = []
    ...
    def _get_mirror_position(self, x: int, y: int) -> tuple:
        mirrored_y = GRID_WIDTH - 1 - y
        mirrored_y = max(6, min(GRID_WIDTH - 1, mirrored_y))
        return (x, mirrored_y)
