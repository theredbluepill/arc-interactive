"""Two-key unlock: pick up key A before passing door A, key B before door B; wrong door without key = lose."""
...
class Ul02UI(RenderableUserDisplay):
    ...
    def render_interface(self, frame):
        ...
        for dy in range(2):
            for dx in range(2):
                frame[h - 3 + dy, w - 5 + dx] = 11 if self._a else 5
                frame[h - 3 + dy, w - 3 + dx] = 11 if self._b else 5
        return frame
...
class Ul02(ARCBaseGame):
    def __init__(self) -> None:
        self._ui = Ul02UI(False, False)
        super().__init__(
            "ul02",
            levels,
    ...
    def step(self) -> None:
        ...
        if sp and "key_a" in sp.tags:
            ...
            self._has_a = True
            self._player.set_position(nx, ny)
            self._ui.update(self._has_a, self._has_b)
