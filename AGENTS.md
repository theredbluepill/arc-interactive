...
3. **ml01**: Laser **raycast** with slash vs backslash mirrors via **`(-dy,-dx)`** vs **`(dy,dx)`**; **ACTION1–4** = **no-op**; **ACTION5** = fire full ray; **ACTION6** = place/cycle on any **valid** cell (**`camera.display_to_grid`**), not on wall / hazard / emitter / receptor — **no** separate **player** sprite (unlike **ml02**/**ml03**); **ACTION7** = **undo** last **5**/**6** (level clone + **`mirror_inv`** + **`_steps`**, stack cleared on **`on_set_level`**, cap **`MAX_UNDO`**). **`Ml01UI`** = inventory + step ticks + **magenta** beam trace (**/** = **15**, **\\** = **7**; steps HUD **10**). **`on_set_level`**: **`emit_dx` / `emit_dy` / `mirror_inv` / `max_steps`** with **`is not None`**. **Preview GIF**: **`render_ml01_gif.py`**. **Registry GIF**: synthetic click ripple (**preview-only**).
4. **ml02**/**ml03**: **technician** moves (**1–4**); **ACTION6** only on **Manhattan-1** cells; **ml03** **removes** every mirror the beam **reflected from** after each shot. **Row-12 “split wall” gaps** with an **east** emitter must leave a **continuous** opening from the beam’s first cell to the gap — otherwise use **column-wall + gap** (like **ml01**). **`_fire_laser`**: use **`break`** (not **`return`**) on OOB / wall / hazard so the post-loop **`hit >= receptors`** win check still runs. **Receptor hit-test**: call **`get_sprite_at(x, y, tag="receptor", ignore_collidable=True)`** *before* the untagged pass — if the **player** stands on a receptor, the **player** sprite can sort above the goal and block counting. **Preview GIFs**: **`render_ml02_gif.py`**, **`render_ml03_gif.py`** (same route; **ml03** shows mirrors vanishing after the L3 shot).
5. **ml04**: **10×10** grid; **`Camera(0,0,G,G,…)`** — **`set_level`** already matches **`G`**; **stepped** bolt (**ACTION5** = one cell); **ACTION6** cycles fixed slots (**/** / **\\** / empty). Optional **`emit_dir`** in **`level.data`** (**0–3** = E/S/W/N); needed for **vertical** shots that stay aligned with column mirrors. **GIF clicks**: after load, camera is **10×10**, so **`grid_to_display`** must use **`CAM_W=CAM_H=10`** (not the pre-resize constructor size). **Preview GIF**: **`render_ml04_gif.py`**.
6. **sf01**: **64×64** camera; **3×3 stencil** clamped to grid; **`ACTION5`** paints; win when **`goal <= painted`** (subset); refresh **`paint`** sprites each stamp. **Registry GIF**: inner paint loop must guard with **`g.level_index == li`** so clearing a level does not keep painting the next. Use **`append_frame_repeats_latest`** for observations so multi-layer actions stay correct.
7. **ll01**: **Conway** on full grid; evaluate **`_alive()`** into a **snapshot** before mutating sprites; **lose** on generation **`== need`** if pattern mismatch, or **`>` need**. **`Ll01UI`**: yellow ticks = generation progress toward **`need_generations`**; blue bottom row = **toggle** budget. **Preview GIF**: **`render_ll01_gif.py`** — paint **`target_cells`** then **`ACTION5` × `need_generations`** (still-life targets stay fixed).
8. **wl01**: Tag player-placed tiles **`mywall`** separately from static **`wall`**; **goal** is **non-collidable** — win by **position match** after movement.
9. **dd01**: **`ACTION5`** handles pickup/drop **or** HUD pad ping when idle (no **`ACTION6`**).
10. **ck01**: **Wire connectivity** = BFS from **`in_port`** into **orth-adjacent `wire`** cells until **`out_port`** is reached.
11. **ph01**: Keep numeric phases in **`self._g`**; **`phase_cell`** sprites for display — **skip `mark`** cells so targets stay visible; blur uses **orthogonal** neighbors only.
12. **bn01**: **`hidden`** targets in **`level.data`**; **`ghost`** sprites only when **`(x,y) ∈ revealed`** (union of beacon disks); **wrong `ACTION6` flag** → **`lose()`** immediately.
